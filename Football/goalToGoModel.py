...
pxp = open("LDGTG.csv","r",encoding="utf-8")

NUM_OF_PLAYS = 14041 #2015 - 2022 excl 2018+2020
...
plays = n.empty((NUM_OF_PLAYS,5),dtype=n.int16)
...
for line in pxp:
    ...
    try:
       ...
       plays[count][1] = (int(line[0])) #distance to first/distance to goal (these are always the same)
       plays[count][2] = int(line[1]) #time remaining in game
       sd = (int(line[8]) - int(line[9])) #score differeential
       plays[count][3] = sd #score differential between teams
       plays[count][4] = int(int(line[1]) < 180) #are there fewer than three minutes left in the half? 
...
if (NEED_NEW_MODEL):
    ...
    pickle.dump(lr, open("gtgmodel.dat","wb"), protocol=4) #opens file and stores the object there
    print("done fitting model!")
else: 
    lr = pickle.load(open("gtgmodel.dat","rb"))
    print("opened the pickle jar")
mm = pickle.load(open("mainmodel.dat","rb")) #to test against the main model and to give probabilities for safety against

#safeties do not appear in the training dataset but are definitely possible
def makeProbArray(down, dist, time, sd, utm):
    output = list(lr.predict_proba([[down, dist, time, sd, utm]])[0])
    output.append(0)
    output[3:7] = output[2:6]
    mm.predict_proba([[down, dist, dist, time, sd, 1, utm]])[0][2] #get safety against probs from main model
    output = n.array(output)
    output /= n.sum(output) #normalize so probs add up to 1.0
    return output

'''Code to plot the various probabilities for first and goal 
all the way down the field based on the determined parameters. '''
def plotProbsandEP(wantPlots):
  ...
  X_MIN = 1
  X_MAX = 21
  for distance in range(X_MIN,X_MAX):
      #data fed to predict_proba: down, distance, time in half, how much is offense leading by, is under 3 minutes?
      predictions = makeProbArray(1,distance,1200,0,0)
      ...
      pointsExpectation.append((predictions[0] * -6.97 + predictions[1] * -3  + predictions[3] * 2 + predictions[4] * 3 + predictions[5] * 6.97))
...
def testOn2018(wantPlots): 
  season18 = open("2018GTG.csv","r",encoding="utf-8")
  ...
  while (playCount < len(plays18)):
      try:
          ...
          play[2] = int(play[1]) #time      
          play[1] = int(play[0]) #distance to goal
          play[0] = int(play[3]) #down
          play[3] = int(play[8]) - int(play[9]) #score differential (wrt the team with possession)
          play[4] = int(play[2] < 180) #play[2] already set to time
          play[5] = int(play[13]) #next score. not used for prediction (duh), only for testing
          plays18[playCount] = play[0:6] 
          ...
          predictions = makeProbArray(play[0],play[1],play[2],play[3],play[4])
